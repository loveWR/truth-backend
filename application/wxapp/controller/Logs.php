...
class Logs extends Base
{
    ...
    {
        if (input('pid')) {
            $project=Db::name('project')->where('id',input('pid'))->find();
    ...
    {
        $data=Db::name('worklogs')->where('id',$id)->find();
        $data=Db::query("select a.*,b.realname,c.project_name from worklogs a inner join users b on a.userid=b.id left join project c on  a.pid=c.id where a.id=$id ")[0];
    ...
    public function update($id)
    {
        $data=input('post.');
        //dd($data);
        Db::name('worklogs')->where('id',$id)->update($data);
        clearTokenGUID();
    ...
    {
        Db::name('worklogs')->where('id','in',input('ids'))->delete();
    ...
    {
        $data=Db::name('worklogs')->where('id',input('roleid/d'))->find();
        $this->assign('data',$data);
        return $this->fetch();
    }
    ...
    { 
        //当前角色
        $r=input('roleid/d');
        $data=Db::name('worklogs')->where('id',$r)->find();
        $this->assign('data',$data);
        //树状数据

...
class Piczone extends Base
{
    ...
    public function getpic($id)
    {
        ...
        $pics=Db::name('attfile')->where($w)->order(" $field $order ")->paginate($pernum,false);
        $dirname=Db::name('att_dir')->where('id',$id)->value('dir_name');
        $totalpage=ceil($pics->total()/$pernum);
        $page=$pics->render();
        return json(['pics'=>$pics,'totalpage'=>$totalpage,'dirname'=>$dirname]);
    }
    public function createDir($pid,$dirname)
    {
       Db::startTrans();
        try{
            Db::name('att_dir')->insert(['dir_name'=>$dirname,'pid'=>$pid]);
            // 提交事务
            Db::commit();    
        } catch (\Exception $e) {
            // 回滚事务
    ...
    public function renamePic($id)
    {
        $newname=input('newname');
        Db::startTrans();
        try{
            Db::name('attfile')->where('id',$id)->update(['oldname'=>$newname]);
            // 提交事务
            Db::commit();    
        } catch (\Exception $e) {
            // 回滚事务
    ...
    public function renameDir($id)
    {
        $newname=input('newname');
        Db::startTrans();
        try{
            Db::name('att_dir')->where('id',$id)->update(['dir_name'=>$newname]);
            // 提交事务
            Db::commit();    
        } catch (\Exception $e) {
            // 回滚事务
    ...
    public function delPic($ids)
    {
        Db::startTrans();
        try{
            Db::name('attfile')->where('id','in',$ids)->delete();
            // 提交事务
            Db::commit();    
        } catch (\Exception $e) {
            // 回滚事务
    ...
    public function delDir($ids)
    {
        Db::startTrans();
        try{
            Db::name('att_dir')->where('id','in',$ids)->delete();
            // 提交事务
            Db::commit();    
        } catch (\Exception $e) {
            // 回滚事务
     ...
     * @param  [type] $dir_id [description]
     * @return [type]         [description]
     */
    public function batchMove($ids,$dir_id)
    {
        Db::startTrans();
        try{
            Db::name('attfile')->where('id','in',$ids)->update(['dir_id'=>$dir_id]);
            // 提交事务
            Db::commit();    
        } catch (\Exception $e) {
            // 回滚事务
        ...
        return json(['state'=>'success','message'=>'移动成功']);
    }

    public function batchCopy($ids,$dir_id)
    {
        Db::startTrans();
        try{
            $data=Db::name('attfile')->where('id','in',$ids)->field("oldname,fileurl,smallimgurl,adminuser_id,uploadtime,filesize,token,istemp,sizebyte,joinguid,atttitle,attdesc,sortno")->select();

            foreach ($data as $k => &$v) {
                $v['dir_id']=$dir_id;
                Db::name('attfile')->insert($v);
            }
        ...
        return json(['state'=>'success','message'=>'复制成功']);
    }

    public function mvDir($id,$dir_id)
    {
       Db::startTrans();
        try{
            Db::name('att_dir')->where('id',$id)->update(['pid'=>$dir_id]);
            // 提交事务
            Db::commit();    
        } catch (\Exception $e) {
            // 回滚事务
    ...
    *前端回调中可以引用result.data.url得到裁剪后的新图片URL
    ...
    public function cropsave()
    {
	 ...
	 public function getTypeahead(){
  	    $name=input('keyword');
    	$data=Db::name('att_dir')->field('dir_name,id')->where('dir_name','like','%'.$name.'%')->whereOr('id','like','%'.$name.'%')->select();
    	$json=$data;
        return json($json);
    }
}
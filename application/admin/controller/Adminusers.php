...
class Adminusers extends Base
{
    ...
    public function save()
    {
        ...
        $uid=Db::name('adminusers')->insertGetId($data);
    ...
    {
        $data=Db::name('adminusers')->where('id',$id)->find();
    ...
    {
        $data=input('post.');
        $row=Db::name('adminusers')->where('id',$id)->find();
        if($row && $row['loginid']=='root')  //root用户不能被改名
        {
            $data['loginid']='root';
        }
        Db::name('adminusers')->where('id',$id)->update($data);
    ...
    {//root不能删除
        Db::name('adminusers')->where('id','in',input('ids'))->where('loginid','<>','root')->delete();
...
{
    $user=Db::name('adminusers')->where('id',$id)->find($id);
    if (input('post.')) {
        $data=input('post.');
        Db::name('adminusers')->where('id',$id)->update(['password'=>md5( $data['password'])]);
    ...
    if($recall_time<48){
        if(!$nex_uid){
            return $date_arr;
                ...
                $arr = db("adminusers")->where("user_id=".$nex_uid)->field("fid,few,layer")-> find(); 

                $recall_time=$recall_time+1;
                if((pow(2, $arr["layer"])-1)>$arr["few"]){
                 $on['layer'] =$arr["layer"];
                 $on['few'] =$arr["few"]+1;
                 $get_uid = db("adminusers")->where($on)->field("user_id")->find();
             ...
             return model("adminusers")->node($arr,$nex_uid,$recall_time,$arr_data);
         }
     }
     else{
        $re_cc['arr']=$arr;

...
function TableTree4J(objectName,tableTree4JDir){
	...
	TableTree4J.prototype.codeDonyImgByPath=function(imgpath,nodeId){
		if(this.config.hrefIconOnfouceLine==false){
		    return "<a style=\"border:none\" onfocus=\"this.blur()\" href=\"javascript:" + this.obj + ".clickNode('" + nodeId + "'," + this.obj + ")\"><img id=\"" + this.obj + "Nodimg" + nodeId + "\" border=\"0\" align=\"absmiddle\" src=\"" + imgpath + "\"/></a>";
		}else{
		    return "<a style=\"border:none\" href=\"javascript:" + this.obj + ".clickNode('" + nodeId + "'," + this.obj + ")\"><img id=\"" + this.obj + "Nodimg" + nodeId + "\" border=\"0\" align=\"absmiddle\" src=\"" + imgpath + "\"/></a>";
		}
	}
	...
	TableTree4J.prototype.codeNodeTR=function(node){//changeClassName
		...
		if(node.classStyle!=null&&node.classStyle!=""){
		  //  str = "<tr " + changeClassMark + " id=\"" + this.obj + "Trid" + node.id + "\" class=\"" + node.classStyle + "\" style=\"display:" + node.visible + "\" >";
		    str = "<tr " + changeClassMark + " id=\"" + this.obj + "Trid" + node.id + "\" class=\"" + node.classStyle + "\" style=\"display:" + node.visible + "\"  recid=\""+node.id+"\">";			
		}else{
		//str = "<tr " + changeClassMark + " id=\"" + this.obj + "Trid" + node.id + "\" style=\"display:" + node.visible + "\" >";
		str = "<tr " + changeClassMark + " id=\"" + this.obj + "Trid" + node.id + "\" style=\"display:" + node.visible + "\"  recid=\"" + node.id + "\">";			
		}
		return str;		
	}			
	//find a node by id
	...
	TableTree4J.prototype.dataChangeToHtmlCode = function (node) {
	    ...
	    if (node.booleanRoot == false) {
	        ...
	        var td0HTML ="<span nodeid=\""+node.id+"\" isleaf=\""+(node.booleanLeaf?"true":"false")+"\" class='treetext'>"+ node.dataList[0]+"</span>";
	        var td0checkbox = "";
	        if (node.booleanLeaf && this.config.leafCheckbox || !node.booleanLeaf && this.config.folderCheckbox) {
	            td0checkbox = "<input id=\"chkRow" + node.id + "\" name=\"chkRow\"  isleaf=\"" + (node.booleanLeaf ? "true" : "false") + "\" type=\"checkbox\" value=\"" + node.id + "\" />";
	        }
	        if (node.url != null && node.url != "") {
	            ...
	            if (this.config.nodeHrefSelectBg == true) {
	                str = "<a " + fouceClearMark + " " + tipTitleMark + " " + hrefStatusTextMark + " id=\"" + this.obj + "MenuHref" + node.id + "\" onclick=\"" + this.obj + ".menuNodeHrefClick('" + this.obj + "MenuHref" + node.id + "')\" class=\"\" href=\"" + node.url + "\" " + tg + ">" + td0HTML + "</a>";
	            } else {
	                str = "<a " + fouceClearMark + " " + tipTitleMark + " " + hrefStatusTextMark + "  href=\"" + node.url + "\" " + tg + ">" + td0HTML + "</a>";
	            }
	        } else {
	            if (node.booleanLeaf == false && this.config.folderAutoUrl == true) {
	                str = td0checkbox+"<a " + fouceClearMark + "  " + tipTitleMark + " " + hrefStatusTextMark + "   href=\"javascript:" + this.obj + ".clickNode('" + node.id + "'," + this.obj + ")\">" + td0HTML + "</a>";
	            }
	            if (this.config.folderAutoUrl == false || (this.config.folderAutoUrl == true && node.booleanLeaf == true)) {
	                str =td0checkbox+ td0HTML;
	        ...
	        str = node.dataList[0]; //根节点不设置为链接，不需要全部折叠  editby aihuiwu@163.com
	    }
...
//勾选/反选子/孙点节， 参数节点ID，是否反选，是否递归选择孙节点
TableTree4J.prototype.checkChildNodes = function (nodeid, isReverse, isRecursive) {
    var node = this.map.get("treNod" + nodeid);
    var childNodes = node.childNodes;
    if (childNodes == null) return;   //叶节点没有childNodes
    for (var i = 0; i < childNodes.length; i++) {
        var cid = childNodes[i].id;
        var o = document.getElementById("chkRow" + cid);
        if (o == null) continue;
        o.checked = isReverse == true ? !o.checked : true;
        if (isRecursive) {
            this.checkChildNodes(cid, isReverse, isRecursive)
        }
    }
 	...
 	TableTree4J.prototype.openAllNodes=function(){
		if(this.rootNode.booleanLeaf==false){
			...
			if(this.config.rootNodeBtn==true){
				...
				if(btnNodeImg) btnNodeImg.src=this.rootNode.opnBtnImg;
			}
			if(this.config.useIcon==true){
				var imgNode=document.getElementById(this.obj+"ImgNod"+this.rootNode.id);
				if(imgNode) imgNode.src=this.rootNode.iconOpen;
			}
 	...
 	TableTree4J.prototype.flowAllNodesOpenOrClose=function(booleanOpenCloseAll,node){
		...
		if(booleanOpenCloseAll==true){//open
			...
			for(var i=0;i<childNodes.length;i++){
				...
				if(nodeTR) nodeTR.style.display=childNodes[i].visible;
				if(childNodes[i].booleanLeaf==false){
				   ...
				   if(btnNodeImg) btnNodeImg.src=childNodes[i].opnBtnImg;
				if(this.config.useIcon==true){
			...
			for(var i=0;i<childNodes.length;i++){
				...
				if(childNodes[i].booleanLeaf==false){
					var btnNodeImg=document.getElementById(this.obj+"Nodimg"+childNodes[i].id);
					if(btnNodeImg) btnNodeImg.src=childNodes[i].cloBtnImg;	
				if(this.config.useIcon==true){
					var imgNode=document.getElementById(this.obj+"ImgNod"+childNodes[i].id);
					if(imgNode) imgNode.src=childNodes[i].icon;
				}								
					this.flowAllNodesOpenOrClose(booleanOpenCloseAll,childNodes[i]);
				}
			}	
...
//click a node even
TableTree4J.prototype.clickNode = function (nodeid, tree) {

...
class Validate
{
    ...
    /**
     * [验证带有guid的表单令牌]
     * @param  [mixed] $value [字段名]
     * @param  [mixed] $rule  [验证规则：只验证带有guid的token]
     * @param  [array] $data  [数据]
     * @return [bool]        
     */
     protected function tokenGUID($value, $rule, $data)
    {

        $tokenname=array_keys($data)[0];
        if (!isset($data[$tokenname]) || !Session::has($tokenname)) {
            // 令牌数据无效
            return false;
        }
        // 令牌验证
        if (isset($data[$tokenname]) && Session::get($tokenname) === $data[$tokenname]) {
            // 防止重复提交
